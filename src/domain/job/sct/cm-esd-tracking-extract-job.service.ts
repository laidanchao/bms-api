...
/**
 * 将法邮轨迹文件(文件在ESENDEO的FTP)上传到s3
 */
@Injectable()
export class CMESDTrackingExtractJob extends FtpJob {
  constructor(
  ...
  async _handleFile(file, source) {
    await this.metadataExtractService.extractTracking(file, this.ftpClient, 'COLISSIMO', {
      ...
      extractedPath: `${this.configService.get('CMESDTrackingExtractJob').ftp.extracted}/${file.name}`,
